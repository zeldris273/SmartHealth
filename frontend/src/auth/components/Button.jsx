...
const Button = forwardRef(({ 
  ...
  className = '', 
  disabled,
  type = 'button',  // ← THÊM default type
  ...props 
}, ref) => {
  
  // Bỏ overflow-hidden — gây lỗi clip trên Firefox Linux
  const baseStyles = "relative flex items-center justify-center font-medium transition-all duration-300 outline-none rounded-xl group";
  ...
  const variants = {
    ...
    glass: "rounded-[5px] bg-[#8f2c24] text-white text-base font-semibold py-2.5 hover:bg-[#7a251e] shadow-md shadow-[#8f2c24]/25 focus:ring-2 focus:ring-[#8f2c24]/40 border-none",
  ...
  return (
    <button
      ref={ref}
      type={type}  // ← DÙNG type prop thay vì hardcode
      disabled={disabled || isLoading}
      className={`${baseStyles} ${variants[variant]} ${sizes[size]} ${disabledStyles} ${className}`}
      {...props}
    >
      {(variant === 'primary' || variant === 'auth' || variant === 'baymax' || variant === 'glass') && (
        // Dùng rounded-xl thay vì inset-0 để tránh vượt ra ngoài khi không có overflow-hidden
        <div className="absolute inset-0 rounded-[inherit] bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      )}
      
      {isLoading && (
        <Loader2 className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-5 h-5 animate-spin" />
      )}
      <span className={`relative flex w-full items-center justify-center gap-2 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
        {children}
      </span>
    </button>
  );
