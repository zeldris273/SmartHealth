...
const stats = [
  { icon: '⚖️', label: 'BMI', value: '22.4', unit: '', status: 'Bình thường', color: 'text-red-500' },
  { icon: '👜', label: 'Cân nặng', value: '68', unit: 'kg', status: 'Ổn định', color: 'text-red-500' },
  { icon: '🔥', label: 'Calories', value: '1,850', unit: 'kcal', status: 'Hôm nay', color: 'text-orange-400' },
  { icon: '❤️', label: 'Sức khỏe', value: '82', unit: '%', status: 'Tốt', color: 'text-green-500' },
];

const features = [
  { icon: '💬', title: 'AI Chat cùng Baymax', desc: 'Trò chuyện với AI để nhận tư vấn và giải đáp mọi thắc mắc về sức khỏe.', link: 'Trải nghiệm ngay' },
  { icon: '📈', title: 'Theo dõi sức khỏe', desc: 'Theo dõi chỉ số cơ thể và lịch sử sức khỏe theo thời gian thực.', link: 'Xem chi tiết' },
  { icon: '🧠', title: 'Phân tích thông minh', desc: 'AI phân tích dữ liệu và đưa ra những gợi ý cải thiện sức khỏe cho bạn.', link: 'Khám phá ngay' },
];
...
const Home = () => {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-red-100 via-rose-50 to-pink-100 transition-all duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className={`transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <p className="text-gray-500 text-base mb-8 leading-relaxed">
              Baymax luôn sẵn sàng đồng hành cùng bạn theo dõi sức khỏe,<br />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-medium text-gray-700">10K+ người dùng</div>
                <div className="text-xs text-gray-400">tin tưởng sử dụng SmartHealth</div>
              </div>
            </div>
          </div>
      ...
      {/* Stats Section */}
      <section className={`px-8 py-8 max-w-7xl mx-auto transition-all duration-700 delay-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-6">Tổng quan sức khỏe</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 border border-gray-100 hover:border-red-200 hover:shadow-md transition-all duration-200">
              <div className="text-2xl mb-3">{s.icon}</div>
              <div className="text-xs text-gray-400 mb-1">{s.label}</div>
              <div className={`text-2xl font-bold ${s.color}`}>{s.value}<span className="text-sm font-normal ml-1">{s.unit}</span></div>
      ...
      {/* Features Section */}
      <section className={`px-8 py-8 max-w-7xl mx-auto pb-16 transition-all duration-700 delay-400 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-6">Tính năng nổi bật</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {features.map((f, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-red-200 hover:shadow-md transition-all duration-200">
              <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center text-2xl mb-4">{f.icon}</div>
              <h3 className="text-base font-semibold text-[#1e293b] mb-2">{f.title}</h3>
